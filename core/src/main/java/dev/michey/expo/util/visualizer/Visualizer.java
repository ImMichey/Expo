...
public class Visualizer {
    ...
    public Visualizer() throws IOException {
        ...
        // read data
        JSONObject obj = new JSONObject(Files.readString(new File("C:\\IDEAProjects\\ProjectV\\exportedData\\div1_ecos_per_rounds_pistols.json").toPath()));
        ...
        LinkedList<VisualizerData2> list = new LinkedList<>();

        for(int i = 0; i < bars; i++) {
            JSONObject dataset = data.getJSONObject(i);
            list.add(new VisualizerData2(dataset));
        }

        list.sort((o1, o2) -> Float.compare(o1.perc, o2.perc));

        float bestPerRound = list.get(0).perc;
        float worstPerRound = list.get(list.size() - 1).perc;

        ExpoLogger.log(worstPerRound + " " + bestPerRound);
        ...
        String header = "[#B6FF00]Project V[WHITE] - Div. 1 Stage 3 - Rounds between each eco (<5000$)";
        ...
        for(int i = 0; i < list.size(); i++) {
            VisualizerData2 v = list.get(list.size() - 1 - i);
            float height = maxHeight * v.perc * 0.01f;
            ...
            // 29/42
            String roundsPlayedText = v.totalRoundsPlayed + "/" + v.ecoRoundsPlayed;
            ...
            float tst = (v.perc - worstPerRound) / (bestPerRound - worstPerRound);

            Color c = colorFrom(tst, Color.LIME, Color.YELLOW, Color.ORANGE, Color.RED);
            ...
            c.a = 0.75f;
            String percAsText = String.format(Locale.US, "%.2f", v.perc);
