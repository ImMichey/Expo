 ...
 * <p>
 * This is an optimized version of the {@link com.badlogic.gdx.graphics.g2d.SpriteBatch} that maintains an texture-cache inside a GL_TEXTURE_2D_ARRAY to combine
...
public class ArrayTextureSpriteBatch implements Batch {
     ...
     *            Framebuffer Objects. Make sure to implement a Fallback to {@link com.badlogic.gdx.graphics.g2d.SpriteBatch} in case Texture Arrays are not
    ...
    public void draw (Texture texture, float x, float y, float width, float height) {
        ...
        final float fy2 = y + height;
        final float u = 0;
        final float v = subImageScaleWidth; // subImageScaleWidth
        final float u2 = subImageScaleHeight; // subImageScaleHeight
        final float v2 = 0;
    ...
    public static final float SEMI_TRANSPARENT_BITS = Color.toFloatBits(0f, 0f, 0f, 0.5f);

    public void drawGradientInverted(TextureRegion region, float x, float y) {
        if (!drawing) throw new IllegalStateException("ArrayTextureSpriteBatch.begin must be called before draw.");
        ...
        float top = BLACK_BITS;
        float bottom = Color.WHITE_FLOAT_BITS;
    ...
    public void drawGradientSemiTransparent(TextureRegion region, float width, float height, Affine2 transform) {
        if (!drawing) throw new IllegalStateException("ArrayTextureSpriteBatch.begin must be called before drawGradientSemiTransparent.");
        ...
        //float color = this.colorPacked;
        float colorBOTTOM = GameSettings.get().enableSoftShadows ? SEMI_TRANSPARENT_BITS : BLACK_BITS;
        float colorTOP = GameSettings.get().enableSoftShadows ? TRANSPARENT_BITS : BLACK_BITS;

        vertices[idx++] = x1;
        vertices[idx++] = y1;
        vertices[idx++] = colorBOTTOM;
        vertices[idx++] = u;
        vertices[idx++] = v;
        vertices[idx++] = ti;
    ...
    private int activateTexture (Texture texture) {
        ...
        //log("ActivateTexture " + texture.getWidth() + "x" + texture.getHeight() + " - " + subImageScaleWidth + "&" + subImageScaleHeight + " - " + invTexWidth + "&" + invTexHeight);
